...
public class Assignment5 {
    ...
    private void initialise() {
        ...
        JButton Ngrams = new JButton("Ngrams");
        Ngrams.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String wap = readFile(new File("./data/war_and_peace.txt"));
                Ngrams ngrams = new Ngrams(wap);
                String[] messages = {"Hurihia to aroaro ki te ra tukuna to atarangi kia taka ki muri i a koe",
                        "Turn your face to the sun and the shadows fall behind you, translation"};
                StringBuilder text = new StringBuilder();
                for (String message : messages) {
                    List<Float> probs = ngrams.findCharProbs(message);
                    float logProb = ngrams.calcTotalLogProb(probs);

                    text.append("Text:").append("\n");
                    text.append(message).append("\n");
                    text.append("Probabilities:").append("\n");
                    for (int i = 0; i < message.length(); i++) {
                        text.append(message.charAt(i)).append(": ").append(probs.get(i)).append("\n");
                    }
                    text.append("Log Probability: ").append(logProb).append("\n");
                }
                textEditor.setText(text.toString());
            }
        });

        JButton boyerMooreBtn = new JButton("Boyer Moor");
        boyerMooreBtn.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String pattern = searchField.getText();
                String text = textEditor.getText();
                int index = new BoyerMoore().search(text, pattern);
